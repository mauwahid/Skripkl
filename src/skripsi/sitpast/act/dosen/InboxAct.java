...
public class InboxAct extends GenericForwardComposer<Component>{
	...
	private void loadPesan(){
		...
		//pesans = query.list();
		pesans = sess.createCriteria(Pesan.class).add(Restrictions.eq("toDosen", dosen))
	...
	class PesanRenderer implements ListitemRenderer<Pesan>{
		...
		public void render(Listitem item, Pesan data, int index)
			...
			if(data.getFromMhs()!=null)
				...
				new Listcell("Prodi : "+data.getFromUser().getNamaLengkap()).setParent(item);
	...
	private void showPesan(Pesan pesan){
		...
		if(pesan.getFromMhs()!=null)
			...
			lbl_dari = new Label("Prodi : "+pesan.getFromUser().getNamaLengkap());
