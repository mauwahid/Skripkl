...
public class MahasiswaSkripsiAct extends GenericForwardComposer<Component> {
	...
	class SkripsiRenderer implements ListitemRenderer<Skripsi>{
		...
		public void render(Listitem item, Skripsi data, int index)
			...
			Button btnValidasi = new Button("Validasi");
			btnValidasi.addEventListener("onClick", new EventListener<Event>() {

				@Override
				public void onEvent(Event event) throws Exception {
					// TODO Auto-generated method stub
					onValidasi(skripsiTemp);
				}
			
			
			});
			
			btnValidasi.setParent(box);
	...
	private void onValidasi(Skripsi skripsi){	
		//element
