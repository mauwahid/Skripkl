...
import skripsi.sitpast.act.mhs.InboxAct.PesanRenderer;
...
public class OutboxAct extends GenericForwardComposer<Component>{
	...
	private void loadPesan(){
		...
		//pesans = query.list();
		pesans = sess.createCriteria(Pesan.class).add(Restrictions.eq("fromMhs", mahasiswa))
	...
	class PesanRenderer implements ListitemRenderer<Pesan>{
		...
		public void render(Listitem item, Pesan data, int index)
			...
			new Listcell(data.getToDosen().getNama()+" , "+data.getToDosen().getGelar()).setParent(item);
	...
	private void showPesan(Pesan pesan){
		...
		lbl_dari = new Label(pesan.getToDosen().getNama()+" , "+pesan.getToDosen().getGelar());
