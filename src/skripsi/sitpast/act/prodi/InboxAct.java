...
public class InboxAct extends GenericForwardComposer<Component>{
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		prodi = (UserTable) session.getAttribute("user");
		loadPesan();
	}
	...
	private void loadPesan(){
		...
		//pesans = query.list();
		pesans = sess.createCriteria(Pesan.class).add(Restrictions.eq("toUser", prodi))
	...
	class PesanRenderer implements ListitemRenderer<Pesan>{
		...
		public void render(Listitem item, Pesan data, int index)
			...
			if(data.getFromMhs()!=null)
				new Listcell("Mahasiswa : "+data.getFromMhs().getNama()).setParent(item);
			else
				new Listcell("Dosen : "+data.getFromDosen().getNama()).setParent(item);
	...
	private void showPesan(Pesan pesan){
		...
		if(pesan.getFromMhs()!=null)
			lbl_dari = new Label("Mahasiswa : "+pesan.getFromMhs().getNama());
		else
			lbl_dari = new Label("Dosen : "+pesan.getFromDosen().getNama());
